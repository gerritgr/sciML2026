# sciML2026
Slides for the scientific machine learning workshop 2026 at DFKI/RPTU 

Slides password is _sciml2026_

## Publications

- **SQUID: A Bayesian Approach for Physics-Informed Event Modeling**  
  Sumantrak Mukherjee, Sebastian Josef Vollmer, Gerrit Großmann  
  *NeurIPS Workshop on Differentiable Systems and Scientific Machine Learning (EurIPS-DSML), 2025*  
  

- **CLAM: Causal Spatial Disaggregation to Infer Local Effects From Coarse Data**  
  Gerrit Großmann, Sumantrak Mukherjee, Sebastian Josef Vollmer  
  *NeurIPS 2025 Workshop on CauScien: Uncovering Causality in Science*

- **Unsupervised Relational Inference Using Masked Reconstruction**  
  Gerrit Großmann, Julian Zimmerlin, Michael Backenköhler, Verena Wolf  
  *Applied Network Science*, 8(1):18, 2023. Springer

- **Network Reconstruction via Sensitivity Analysis**  
  Gerrit Großmann  
  *Extended Abstract, Complex Networks Conference*, 2023
